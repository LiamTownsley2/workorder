...
export default button('create_plugin', async ({ client, interaction }) => {
    if (!interaction.guild) return;
    await interaction.deferReply({ ephemeral: true });

    const ticket = await createTicket(interaction.guild, interaction.user, 'new_plugin');
    ...
    await handleTicket(interaction.guild, ticket, interaction.user, 'new_plugin');
})